...
def pyfile_add_comments(py_filename: str, messages: Dict[int, Set[str]], signature) -> None:
        ...
        for line_no, line in enumerate(py_file):
            ...
            if pylint_line_no in messages:
                if "missing-module-docstring" in messages[pylint_line_no]:
                    first_line = f"# pylint: disable=missing-module-docstring{signature}{EOL}"
                ...
                if sorted_messages:
                    line = (
                        f"{line.rstrip()}"
                        f"  # pylint: disable={','.join(sorted_messages)}{signature}{EOL}"
                    )
            out_file.write(line)
...
def apply(pylint_logfile: str, signature) -> None:
        ...
        for line in logfile:
            ...
            if py_filename != active_py_filename:
                ...
                if active_py_filename is not None:
                    pyfile_add_comments(active_py_filename, messages, signature)
...
def reset(py_filename: str, signature, used_signature=False) -> None:
    ...
    with open(py_filename, "r", encoding="utf-8") as py_file, \
        ...
        for line in py_file:
            if line.rstrip() == f"# pylint: disable=missing-module-docstring{signature}":
                continue

            # Do not remove comments that weren't generated by pylint-silent (if --signature enabled)
            if used_signature and signature not in line:
                out_file.write(line)
                continue
...
def statistics(py_filenames: List[str], signature) -> None:
    ...
    for py_filename in py_filenames:
        ...
        with open(py_filename, "r", encoding="utf-8") as py_file:
            ...
            for line in py_file:
                ...
                if (
                    comment_pos > 0
                    or line.rstrip() == f"# pylint: disable=missing-module-docstring{signature}"
                ):
