...
def pyfile_add_comments(
    py_filename: str, messages: Dict[int, Set[str]], signature: str
) -> None:
        ...
        for line_no, line in enumerate(py_file):
            ...
            if pylint_line_no in messages:
                if "missing-module-docstring" in messages[pylint_line_no]:
                    first_line = (
                        f"# pylint: disable=missing-module-docstring{signature}{EOL}"
                    )
                ...
                if sorted_messages:
                    line = (
                        f"{line.rstrip()}"
                        f"  # pylint: disable={','.join(sorted_messages)}"
                        f"{signature}{EOL}"
                    )
            out_file.write(line)
...
def apply(pylint_logfile: str, signature: str) -> None:
